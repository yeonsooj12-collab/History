...
const GROUPS = {
  ...
  core: {
    title: "조건",
    kicker: "1단계",
    description: "먼저 사회를 밀어붙이는 내부 압력과 외부 조건을 고릅니다.",
  },
  detail: {
    title: "구조",
    kicker: "2단계 · 선택",
    description: "누가 대응하고, 어떤 집단이 어떤 기준으로 분류되는지 정합니다.",
  },
  amplifier: {
    title: "작동과 결과",
    kicker: "3단계 · 선택",
    description: "통치 수단, 정당화 언어, 실제 변화, 저항과 균열을 정합니다.",
  },
};
...
function renderResultPanel() {
  ...
  if (!state.result) {
    panel.append(createEl("p", "help-text", "조건을 작성한 뒤 세계사 렌즈 요청을 만들어보세요."));
...
export function updateProgress(group) {
  ...
  if (group === "scope") {
    ...
    return count > 0 ? `범위 힌트 ${count}/${keys.length} 작성` : "비워도 됨";
  }
  const label = group === "core" ? "조건" : group === "detail" ? "구조" : "작동과 결과";
